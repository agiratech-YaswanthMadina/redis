...
function CrudRouter(model, hooks, messageName = '') {
  ...
  router.post(
    ...
    function (req, res) {
      ...
      return res.status(200).send(`msg: New Employee with ID ${record._id} has been created Successfully`);
    }
  );
  ...
  router.delete(
    ...
    function (req, res) {
      let { record } = res.locals || {};
      return res.status(200).send(`msg: ID ${record._id} has been Deleted Successfully`);
    }
  );
